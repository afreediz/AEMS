import React, { useState } from 'react';
import {Link, useLocation, useNavigate} from 'react-router-dom'
import './sidebar.css'

function Sidebar() {
    const location = useLocation()
    const navigate = useNavigate()
    const [showLogout, setShowLogout] = useState(false)
    const [active, setActive] = useState(location.pathname.split('/').pop())
    return (
        <div className="main-container">
            <div className="navcontainer ">
                <nav className="nav">
                    <div className="nav-upper-options">
                        <Link  to='/admin/' onClick={()=>{setActive('dashboard')}}>
                        <div className={`nav-option ${active=="dashboard" ? "bg-blue-800 text-white" : ""}`}>
                            <img src="https://cdn-icons-png.flaticon.com/512/4305/4305636.png" className="nav-img" alt="DASHBOARD" />
                            <h3> DASHBOARD</h3>
                        </div>
                        </Link>
                        <Link  to='/admin/participants' onClick={()=>{setActive('participants')}} >
                        <div className={`nav-option ${active=="participants" ? "bg-blue-800 text-white" : ""}`}>
                        <img src= 
                        "data:image/jpeg;base64,"
							className="nav-img"
							alt="PARTICIPANTS" /> 
                            <h3>PARTICIPANTS</h3>
                        </div>
                        </Link>
                        <Link  to='/admin/events' onClick={()=>{setActive('events')}} >
                        <div className={`nav-option ${active=="events" ? "bg-blue-800 text-white" : ""}`}>
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT-fbalSFDxWY4rhmWQlDMVVnIFv96NGMpKSw&s" className="nav-img" alt="EVENTS" />
                            <h3>EVENTS</h3>
                        </div>
                        </Link>
                        <Link  to='/admin/certificate' onClick={()=>{setActive('certificate')}} >
                        <div className={`nav-option ${active=="certificate" ? "bg-blue-800 text-white" : ""}`}>
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSy-ZwOpmkc7Q86s0b7qJgmu3OqPV_nsBrYNg&s" className="nav-img" alt="CERTIFICATE" />
                            <h3>CERTIFICATE</h3>
                        </div>
                        </Link>
                        <Link  to='/admin/mail' onClick={()=>{setActive('mail')}} >
                        <div className={`nav-option ${active=="mail" ? "bg-blue-800 text-white" : ""}`}>
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQHJ8xZvVENfx3Hcn6s7X5NU9RxCOoFh-V5i5Oh708IJg&s" className="nav-img" alt="EMAIL" />
                            <h3> EMAIL</h3>
                        </div>
                        </Link>
                        <Link  to='/admin/categories' onClick={()=>{setActive('categories')}} >
                        <div className={`nav-option ${active=="categories" ? "bg-blue-800 text-white" : ""}`}>
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS6R4yHoJ1m5NOcEUvXPjKEHNJ25T94pM2jfnM8N4g3Og&s" className="nav-img" alt="EMAIL" />
                            <h3> CATEGORIES</h3>
                        </div>
                        </Link>
                        <Link  to='/admin/uiux' onClick={()=>{setActive('uiux')}} >
                        <div className={`nav-option ${active=="uiux" ? "bg-blue-800 text-white" : ""}`}>
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS6R4yHoJ1m5NOcEUvXPjKEHNJ25T94pM2jfnM8N4g3Og&s" className="nav-img" alt="EMAIL" />
                            <h3> MANAGE UIUX</h3>
                        </div>
                        </Link>
                        <div className="nav-option logout" onClick={()=>{
                            setShowLogout(true)
                        }}>
                            <img src="https://media.geeksforgeeks.org/wp-content/uploads/20221210183321/7.png" className="nav-img" alt="logout" />
                            <button><h3>Logout</h3></button>
                        </div>
                    </div>
                </nav>
            </div>
            {showLogout &&
            <div className="absolute w-full h-full items-centerjustify-center z-40">
                <div className=" shadow-black shadow-2xl outline border-2 border-slate-700  logout-box w-50vh bg-white my-44 mx-auto text-center p-12">
                    <h2 className='text-4xl my-8'>Logout</h2>
                    <p className='text-3xl'>Are you sure you want to logout?</p>
                    <div className="button-group">
                        <button className="yes-btn text-3xl bg-black text-white py-4 px-8 m-4" onClick={()=>{
                            localStorage.removeItem('token')
                            setShowLogout(false);
                            navigate('/login')
                        }}>Yes</button>
                        <button className="no-btn text-3xl bg-black text-white py-4 px-8 m-4" onClick={()=>{
                            setShowLogout(false);
                        }}>No</button>
                    </div>
                </div>
            </div>
            }
        </div>
    );
}

export default Sidebar;
